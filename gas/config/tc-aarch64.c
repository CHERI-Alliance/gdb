...
#define MAP_CUR_INSN (AARCH64_CPU_HAS_FEATURE (cpu_variant, C64)	\
		      ? MAP_C64 : MAP_INSN)

#define IS_C64 (AARCH64_CPU_HAS_FEATURE (cpu_variant, C64) \
		? 1 : 0)
...
enum aarch64_abi_type
{
  ...
  AARCH64_ABI_ILP32 = 2,
  AARCH64_ABI_LLP64 = 3,
  AARCH64_ABI_PURECAP = 4,
  AARCH64_ABI_HYBRID = 5,
};
  ...
  for (i = 0; operands[i] != AARCH64_OPND_NIL; i++)
	...
	case AARCH64_OPND_Cat_SYS:
	  ...
	  if (opcode->op == OP_MOV_C_ZR && operands[i] == AARCH64_OPND_Can
	      && reg->number != 31)
	    {
	      set_fatal_syntax_error (_(get_reg_expected_msg (REG_TYPE_CA_Z, 0)));
	      goto failure;
	    }
	  if (reg->number > 31)
	    {
	      set_fatal_syntax_error (_(get_reg_expected_msg (REG_TYPE_CA_N, 0)));
	      goto failure;
	    }
	  info->reg.regno = reg->number;
		...
		switch (opcode->iclass)
		  ...
		  case condbranch:
		    ...
		    inst.reloc.type = (c64 ? BFD_RELOC_MORELLO_BRANCH19
				       : (operands[i] == AARCH64_OPND_ADDR_PCREL9)
...
tc_aarch64_fde_entry_init_extra(struct fde_entry *fde)
{
  ...
  fde->entry_extras.memtag_frame_p = false;
