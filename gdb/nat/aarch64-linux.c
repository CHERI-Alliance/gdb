...
void
aarch64_compat64_siginfo_from_siginfo (compat64_siginfo_t *to, siginfo_t *from)
{
  memset (to, 0, sizeof (*to));
  ...
  if (to->si_code == SI_TIMER)
    {
      to->cpt_si_timerid = from->si_timerid;
      to->cpt_si_overrun = from->si_overrun;
      to->cpt_si_ptr = (__cheri_addr int64_t) from->si_ptr;
    }
  else if (to->si_code == SI_USER)
    {
      to->cpt_si_pid = from->si_pid;
	...
	case SIGSEGV:
	case SIGBUS:
	  to->cpt_si_addr = (__cheri_addr int64_t) from->si_addr;
	  break;
	case SIGPOLL:
	  to->cpt_si_band = from->si_band;
	  to->cpt_si_fd = from->si_fd;
...
void
aarch64_siginfo_from_compat64_siginfo (siginfo_t *to, compat64_siginfo_t *from)
{
  memset (to, 0, sizeof (*to));
...
void
aarch64_compat64c_siginfo_from_siginfo (compat64c_siginfo_t *to,
					siginfo_t *from)
{
  memset (to, 0, sizeof (*to));
...
void
aarch64_siginfo_from_compat64c_siginfo (siginfo_t *to,
