...
struct riscv_creg_feature : public riscv_register_feature
{
	      ...
	      struct riscv_gdbarch_features *features) const
  {
    ...
    for (const auto &reg : m_registers)
      {
	bool found = reg.check (tdesc_data, feature_xcheri, aliases);
	...
	case riscv_arg_info::location::in_reg:
	  {
		...
		regcache->cooked_read_part (regnum, 0,
					    ...
					    ptr);
		if (abi_val->tagged ())
		  abi_val->set_tag (regcache->raw_collect_tag (regnum));
	      }
		...
		if (register_has_tag (gdbarch, regnum))
		  {
		    if (write_value->tagged ())
		      regcache->raw_supply_tag (regnum, write_value->tag ());
		    else
				      ...
				      struct ui_file *stream)
{
  ...
  gdb_printf (stream, "{tag = %d, address = %s, permissions = {[%s], ",
	      cap->cr_tag, paddress (gdbarch, cap->address ()), perms.c_str ());
  gdb_printf (stream, "flags = %d, ", cap->flags ());
  if (cap->is_sealed ())
    gdb_printf (stream, "otype = %s, ", core_addr_to_string_nz (cap->type ()));
  gdb_printf (stream, "range = [%s - %s)}}", paddress (gdbarch, cap->base ()),
	      paddress (gdbarch, cap->top64 ()));
}
