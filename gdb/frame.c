...
frame_unwind_register_unsigned (const frame_info_ptr &next_frame, int regnum)
{
  ...
  if (type->code () == TYPE_CODE_CAPABILITY || TYPE_CAPABILITY (type))
    ...
    r = extract_unsigned_integer (value->contents_all ().data (),
				  size, byte_order);
  ...
  switch (lval)
	       ...
	       && !gdbarch_pseudo_register_write_value_p (gdbarch)))
	{
	  ...
	  if (register_has_tag (regcache->arch (), realnum))
		...
		tag = false;
	      regcache->raw_supply_tag (realnum, tag);
	  }
...
void
put_frame_register_value (const frame_info_ptr &next_frame, int regnum,
